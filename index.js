...
class Router extends React.Component {
  ...
  onPopToRoute(nextRoute, navigator) {
    navigator.popToRoute(Object.assign(nextRoute));
  }
  ...
  onResetToRoute(nextRoute, navigator) {
    navigator.resetTo(Object.assign(nextRoute, { index: 0 }));
  }
  ...
  render() {
    ...
    if (StatusBar) {
      ...
      if (this.state.route && this.state.route.statusBarProps) {
        statusBarProps = _.defaults(
          this.state.route.statusBarProps,
          statusBarProps
        );
      }
      ...
      if (Platform.OS === 'android') {
        if (
          !_.has(statusBarProps, 'backgroundColor') &&
          !_.has(statusBarProps, 'translucent')
        ) {
          let backgroundColor;

          if (this.state.route && this.state.route.headerStyle) {
            // If current route has specific header style
            const stateHeaderStyle = StyleSheet.flatten(
              this.state.route.headerStyle
            );
    ...
    return (
      <View style={[{ flex: 1 },this.props.sceneStyle]}>
        {statusBar}
        <Navigator
          ref="navigator"
